...
public class Mesh {
	...
	private Mesh(Function<Indices, CellData> creator, double cellSizeEast,
	...
	public Mesh step(double timeStepSeconds) {
		...
		return new Mesh(i -> new CellData() {
			...
			private VelocityPressure velocityPressure() {
				// retrieve or if not present calculate, cache and return
				if (vp.get() == null) {
					vp.compareAndSet(null,
							solver.step(m.cell(i), timeStepSeconds));
				}
				return vp.get();
			}
