...
public class NewtonsMethod {
	...
	public static Optional<Double> solve(Function<Double, Double> f,
			double initialValue, double delta, double precision,
			int maxIterations) {
		...
		while (Math.abs(fx) > precision && i <= maxIterations) {
			double gradient = (f.apply(x + delta) - fx) / delta;
			if (gradient == 0)
	...
	public static final class Builder {
		...
		public Optional<Double> solve() {
			return NewtonsMethod.solve(f, initialValue, delta, precision,
					maxIterations);
		}
