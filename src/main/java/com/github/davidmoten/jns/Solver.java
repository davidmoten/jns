...
public class Solver {
	...
	private final static Set<Direction> DIRECTIONS = Collections
			.unmodifiableSet(new HashSet<Direction>(Arrays.asList(
	...
	public Cell step(Cell cell, double timeStepSeconds) {
		...
		Function<Double, Double> f = getContinuityFunction(cell, v,
				timeStepSeconds);
	...
	private double solveForPressure(Cell cell,
		...
		Optional<Double> p = NewtonsMethod.solve(continuityFunction,
	...
	private Vector getVelocityGradient(Cell cell, Direction direction) {
		Function<Direction, Double> gradient = d -> getGradient(cell,
		...
		return pressureLaplacian
				+ DIRECTIONS.stream()
						...
						.map(d -> override.velocity().value(d)
								* getGradient(cell, d, gradientDot(d),
	...
	private Vector getPressureGradient2nd(Cell cell, Cell override) {
		Function<Direction, Double> f = d -> getGradient(cell, d,
