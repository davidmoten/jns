...
public class NewtonsMethodTest {
    ...
    public void testCanSolveForSquareRootOf2() {
        ...
        Optional<Double> r = NewtonsMethod.solve(x -> x * x - 2, 1, 0.1, precision, 100);
    ...
    public void testCanSolveForSquareRootOf2UsingBuilder() {
        double precision = 0.00001;
        Optional<Double> r = NewtonsMethod.solver().function(x -> x * x - 2).initialValue(1.0)
                .delta(0.1).precision(precision).maxIterations(100).solve();
        assertEquals(Math.sqrt(2.0), r.get(), precision);
    }
    ...
    public void testCanSolveForSquareRootOf2UsingBuilderAndDefaults() {
        double precision = 0.00001;
        Optional<Double> r = NewtonsMethod.solver().function(x -> x * x - 2).solve();
        assertEquals(Math.sqrt(2.0), r.get(), precision);
    }
    ...
    public void testCannotMeetPrecision() {
        double precision = 0.00001;
        Optional<Double> r = NewtonsMethod.solve(x -> x * x - 2, 1, 0.1, precision, 1);
