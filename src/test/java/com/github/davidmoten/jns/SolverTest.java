...
public class SolverTest {
	...
	public void testTransform2D() {
		...
		log.info("cells={}.{},{}", t.c1().position(), t.c2().position(), t.c3()
				.position());
	...
	public void testObstacleToValue() {
		...
		checkEquals(obstacle.position(), Solver.obstacleToValue(obstacle, cell)
				.position(), VELOCITY_PRECISION);
	}
	...
	public void testCellAtBoundaryOfWhirpoolMeshReturnsConstantVelocity() {
		...
		checkEquals(Vector.create(1, 0, 0),
				solver.getVelocityAfterTime(cell, 1), VELOCITY_PRECISION);
	}
	...
	private Mesh checkNoChangeAfterStep(int eastIndex, int northIndex,
			int upIndex) {
		final Solver solver = new Solver();
