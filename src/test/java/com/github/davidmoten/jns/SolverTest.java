...
public class SolverTest {
    ...
    public void testSolverWithRegularGridStillWater() {
        final Solver solver = new Solver();
        final RegularGrid grid = RegularGrid.builder().cellSize(1).cellsEast(10).cellsNorth(10)
                .cellsUp(10).density(1025).viscosity(30).build();
        final Cell cell = grid.cell(5, 5, 5);
        final Cell result = solver.step(cell, 1);
    }
    ...
    private static Builder builder() {
        return CellImpl.builder().pressure(pressureAtDepth(1)).density(Util.DENSITY_KG_PER_M3)
                .viscosity(30).type(CellType.FLUID).velocity(0, 0, 0);
    }
