...
public class SolverTest {
    ...
    private static Cell createCell2() {
        ...
        final CellImpl up = builder().position(0, 0, 0).pressure(pressureAtDepth(0)).build();
        final CellImpl down = builder().position(0, 0, -2).pressure(pressureAtDepth(2)).build();
        final CellImpl centre = builder().position(0, 0, -1).north(north).south(south).east(east)
                .west(west).up(up).down(down).build();
        north.south(centre);
        south.north(centre);
        east.west(centre);
    ...
    private static Builder builder() {
        return CellImpl.builder().pressure(pressureAtDepth(1)).density(DENSITY_KG_PER_M3)
                .temperature(293).viscosity(30).type(CellType.FLUID).velocity(0, 0, 0);
    }
